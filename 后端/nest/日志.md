...
app.useLogger(app.get(WINSTON_MODULE_NEST_PROVIDER)); // 获取logger实例
```
...
main.ts 中`app.useLogger(app.get(WINSTON_MODULE_NEST_PROVIDER)); `是注册全局的`WINSTON_MODULE_NEST_PROVIDER`logger，但是在注册之前，**需要在`app.module.ts`中将`loggermodule`导入来初始化winston的配置，否则`app.get(WINSTON_MODULE_NEST_PROVIDER)`是获取不到WINSTON_MODULE_NEST_PROVIDER的值，就会导致全局logger注册失败**；
